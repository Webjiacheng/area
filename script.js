...
//声明一个变量储存选择的省,由于选择区镇时需要用到这个key
var index1=0;

//一加载页面就把第一栏下拉框加载完毕
region['0'].forEach(function (val,i) {
    var op=new Option(val,val);
    province.options[province.length]=op;
})

//当改变第一个下拉框时，触发change()事件
var change = function (src) {
    city.innerHTML="";//将地市下拉框的内容清空
    //求出省的索引index
    region['0'].forEach(function (val,i) {
        if(val==src){
            index1=i;
            region[0+'_'+i].forEach(function (val) {
                var op=new Option(val,val);
                city.options[city.length]=op;//根据选择的省份初始化地级市下拉框
            });
            county.innerHTML="";//将区镇下拉框的内容清空
            region[0+'_'+i+'_'+0].forEach(function (val) {
                var op=new Option(val,val);
                county.options[county.length]=op;//根据选择的地级市初始化区镇下拉框
            })
        }
    })
}
...
//当改变第二个下拉框时，触发countyChange()事件
var countyChange=function (src2) {
    county.innerHTML="";//将区镇下拉框的内容清空
    for(var i=0;i<34;i++){
        region[0+'_'+i].forEach(function (val,i) {
            if(src2==val){
                region[0+'_'+index1+'_'+i].forEach(function (val) {
                    var op=new Option(val,val);
                    county.options[county.length]=op;//根据选择的地级市初始化区镇下拉框
                })
            }
